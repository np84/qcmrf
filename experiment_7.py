...
def genCircuit(beta, CLIQUES):
	...
	for C in CLIQUES:
		...
		u = conjugateBlocks(CLIQUE_FACTOR).to_circuit().to_instruction(label='U_C('+','.join(str(v) for v in C)+')')
		...
		circ.h(qr[n+i])
		circ.measure([n+i],[n+i]) # real part extraction successful if measured 0
